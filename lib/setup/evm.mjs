...
const CONTRACTS_SOL_DIR = join(ROOT, 'contracts', 'sol');
const EVM_ADDRESSES_FILE = join(ROOT, '_build', 'evm-addresses.json');
...
// Milestone 4 (USDT/generic-ERC20 DEX-hop support, see setupUsdtDexHop()) — Boltz itself never
// learns about any of this (USDT, Router, Permit2, the mock DEX); it's entirely client-side, so
// none of it touches BOLTZ_CONFIG/buildArbitrumConfigToml below.
const USDT_SYMBOL = 'USDT';
const USDT_DECIMALS = 6;
// 1,000,000 USDT at 6 decimals.
const USDT_INITIAL_SUPPLY = '1000000000000';
// Plenty of tBTC (8 decimals) for the mock USDT->tBTC dex to pay out swaps with.
const MOCK_DEX_TBTC_FUNDING = '1000000000000';
...
[arbitrum]
# Must be ws://, not http://: boltzd's consolidated event handler (used for
# ERC20Swap Lockup/Claim/Refund event subscriptions) fails app init with
# "Could not initialize Boltz: at least one WebSocket provider is needed"
# against a plain HTTP JsonRpcProvider — confirmed via a real CI run, the
# basic RPC connection succeeds fine over HTTP ("Connected to Arbitrum RPC
# provider ... on network: 31337"), it's specifically the event-subscription
# path that requires a WebSocketProvider. Anvil serves both protocols on the
# same port, so just changing the scheme is enough.
providerEndpoint = "ws://anvil:8545"
...
# Same ws:// requirement as the L2 providerEndpoint above — confirmed via a
# second CI run: with only the L2 side on ws://, boltzd still failed with
# "at least one WebSocket provider is needed" (the L2 InjectedProvider
# connected fine over ws:// first — "Connected to 1 Arbitrum RPC providers"
# — immediately before the same fatal error), so the check must span both
# the L1 and L2 provider sets, not just the one used for event subscriptions.
...
/**
 * Extracts Permit2's real canonical address + real runtime bytecode straight out of the
 * vendored contracts/sol/permit2/test/utils/DeployPermit2.sol (Uniswap/permit2's own Foundry
 * test helper, which `vm.etch()`s this same bytecode at this same address) — never hand-copy
 * this address from memory: an almost-identical but truncated-by-one-character address was
 * nearly used in place of the real one earlier in this project, caught only by counting hex
 * digits. The vendored .sol file is the single source of truth.
 */
function readPermit2Deployment() {
  const source = readFileSync(join(CONTRACTS_SOL_DIR, 'permit2', 'test', 'utils', 'DeployPermit2.sol'), 'utf8');
  const address = source.match(/0x[0-9A-Fa-f]{40}/)?.[0];
  const bytecode = source.match(/hex"([0-9a-fA-F]+)"/)?.[1];
  if (!address || !bytecode) fail('Could not extract Permit2 address/bytecode from DeployPermit2.sol — has its format changed upstream?');
...
export async function setupEvm() {
  ...
  const usdtDexHop = await setupUsdtDexHop({ tbtcAddress });

  const addresses = { etherSwapAddress, erc20SwapAddress, tbtcAddress, ...usdtDexHop };
  mkdirSync(join(ROOT, '_build'), { recursive: true });
  writeFileSync(EVM_ADDRESSES_FILE, JSON.stringify(addresses, null, 2));
  log(`  Wrote EVM contract addresses to ${EVM_ADDRESSES_FILE}`);

  return addresses;
}

/**
 * Milestone 4: deploys a second TestERC20 (as a USDT stand-in), real Permit2 (injected via
 * anvil_setCode from vendored runtime bytecode — see readPermit2Deployment()), Router, and a
 * MockERC20Dex (funded with tBTC so it can pay out USDT->tBTC swaps) — everything
 * NArk.Tests.End2End's future USDT E2E test needs, discovered via _build/evm-addresses.json
 * (see setupEvm()) rather than Boltz's API, since Boltz never learns about any of this.
 */
async function setupUsdtDexHop({ tbtcAddress }) {
  log('Deploying USDT + Permit2 + Router + MockERC20Dex to Anvil (Milestone 4 DEX-hop)...');

  const usdtAddress = deployRaw(
    'TestERC20.bin',
    'TestERC20 (USDT)',
    'constructor(string,string,uint8,uint256)',
    ['Test USDT', USDT_SYMBOL, String(USDT_DECIMALS), USDT_INITIAL_SUPPLY],
  );
  log(`  USDT (TestERC20) deployed at ${usdtAddress}`);

  const { address: permit2Address, bytecode: permit2Bytecode } = readPermit2Deployment();
  const setCodeRes = cast(['rpc', 'anvil_setCode', permit2Address, `0x${permit2Bytecode}`]);
  if (setCodeRes.code !== 0) fail(`Failed to inject Permit2 bytecode: ${setCodeRes.stderr || setCodeRes.stdout}`);
  log(`  Permit2 deployed at its real canonical address ${permit2Address}`);

  // Router's constructor also requires an EtherSwap-shaped address, but nothing in the
  // ERC20-only DEX-hop flow (executeAndLockERC20WithPermit2/claimERC20Execute) ever touches it
  // (see NArk.Swaps.Evm/Contracts/Sol/Router.sol) — reusing erc20SwapAddress there is
  // structurally valid and avoids deploying yet another throwaway contract.
  const routerAddress = deployRaw(
    'Router.bin',
    'Router',
    'constructor(address,address,address)',
    [process.env.EVM_ERC20SWAP_ADDRESS, process.env.EVM_ERC20SWAP_ADDRESS, permit2Address],
  );
  log(`  Router deployed at ${routerAddress}`);

  const mockDexAddress = deployRaw(
    'MockErc20Dex.bin',
    'MockERC20Dex (USDT->TBTC)',
    'constructor(address,address)',
    [usdtAddress, tbtcAddress],
  );
  log(`  MockERC20Dex (USDT->TBTC, 1:1) deployed at ${mockDexAddress}`);

  const fundRes = cast([
    'send', '--private-key', DEPLOYER_KEY, tbtcAddress,
    'transfer(address,uint256)', mockDexAddress, MOCK_DEX_TBTC_FUNDING,
  ]);
  if (fundRes.code !== 0) fail(`Failed to fund MockERC20Dex with tBTC: ${fundRes.stderr || fundRes.stdout}`);
  log(`  Funded MockERC20Dex with ${MOCK_DEX_TBTC_FUNDING} tBTC base units`);

  process.env.EVM_USDT_ADDRESS = usdtAddress;
  process.env.EVM_PERMIT2_ADDRESS = permit2Address;
  process.env.EVM_ROUTER_ADDRESS = routerAddress;
  process.env.EVM_MOCK_DEX_ADDRESS = mockDexAddress;

  return { usdtAddress, permit2Address, routerAddress, mockDexAddress };
}
